...
<div class="container mx-auto px-4 py-8">
    <h1 class="text-3xl font-bold mb-6">Quotes</h1>
    ...
    <div class="bg-white rounded-lg shadow-sm p-4 mb-6 flex flex-wrap items-center justify-between gap-4">
        ...
        <a href="/quotes/new" class="inline-flex items-center px-4 py-2 bg-blue-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-blue-700 active:bg-blue-900 focus:outline-none focus:border-blue-900 focus:ring ring-blue-300 disabled:opacity-25 transition ease-in-out duration-150">
            Create Quote
        </a>
    </div>
        ...
        <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
                <tr>
                    ...
                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">配信回数</th>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                ...
                <tr>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $quote->getNo() }}</td>
                    <td class="px-6 py-4 text-sm text-gray-900">
                        <div class="max-w-md truncate" title="{{ $quote->getMessage() }}">
                            {{ $quote->getMessage() }}
                        </div>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $quote->getAuthor() }}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 text-center">{{ $quote->getDeliveredCount() }}</td>
    ...
    <div class="mt-8 flex justify-center">
        <nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination">
            ...
            @for ($i = $start; $i <= $end; $i++)
                <a href="?page={{ $i }}" class="relative inline-flex items-center px-4 py-2 border @if($i == $page) border-blue-500 bg-blue-50 text-blue-600 @else border-gray-300 bg-white text-gray-700 hover:bg-gray-50 @endif text-sm font-medium">
                    {{ $i }}
                </a>
            @endfor
